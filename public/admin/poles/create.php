...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? null, 'pole_form')) {
    ...
    if ($data['code'] !== '' && $repo->poleCodeExists($data['code'], $edit ? $id : 0)) {
    ...
    if (empty($errors)) {
        ...
        if ($edit) {
            $repo->updatePole($id, $data['code'], $data['name'], $desc);
            set_flash('success', 'Polo aggiornato.');
        } else {
            $repo->createPole($data['code'], $data['name'], $desc);
...
$title      = $edit ? 'Modifica polo' : 'Nuovo polo';
$submitLabel = $edit ? 'Salva modifiche' : 'Crea polo';
$actionUrl  = $edit
    ? url_for('admin/poles/create.php?id=' . $id)
    : url_for('admin/poles/create.php');
...
$html .= '<a href="' . e(url_for('admin/poles/')) . '">Annulla</a>';
$html .= '</div>';
$html .= '</form>';

render_page_backend($title, $html, [], 'admin.poles.manage');
