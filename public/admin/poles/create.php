...
if (($_SERVER["REQUEST_METHOD"] ?? "GET") === "POST") {
    if (!verify_csrf_token((string) ($_POST["csrf_token"] ?? ""), "pole_form")) {
    ...
    if ($data["code"] !== "" && pole_code_exists($data["code"], $edit ? $id : 0)) {
        $errors[] = "Il codice è già in uso.";
    }

    if ($errors === []) {
        if ($edit) {
            pole_update($id, $data["code"], $data["name"], $data["description"]);
            set_flash("success", "Polo aggiornato.");
        } else {
            pole_create($data["code"], $data["name"], $data["description"]);
            set_flash("success", "Polo creato.");
        }
        redirect(url_for("admin/poles/index.php"));
    }
}
$actionUrl = $edit ? url_for("admin/poles/create.php?id=" . $id) : url_for("admin/poles/create.php");
$html = "<div class=\"admin-toolbar\"><h1>" . ($edit ? "Modifica polo" : "Nuovo polo") . "</h1></div>";
if ($errors !== []) {
...
$html .= "<section class=\"panel form-panel\"><form class=\"admin-form\" method=\"post\" action=\"" . e($actionUrl) . "\">"
    ...
    . "<div class=\"form-actions\"><button class=\"button-primary\" type=\"submit\">" . ($edit ? "Salva modifiche" : "Crea polo") . "</button> "
    . "<a href=\"" . e(url_for("admin/poles/index.php")) . "\">Annulla</a></div>"
    . "</form></section>";
render_admin_page($edit ? "Modifica polo" : "Nuovo polo", $html, "admin.poles.index");
