...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($errors === [] && (new UserRepository())->emailExists($email)) {
    ...
    if ($errors === []) {
        try {
            ...
            if ($newUser !== null) {
                sync_session_favorites_to_db((int) $newUser['id']);
            }
...
if ($errors !== []) {
    ...
    $errorHtml = '<div class="alert alert-danger" role="alert"><ul class="alert-list">' . $items . '</ul></div>';
}

$content = render_template('frontend/register', [
...
render_page_frontend('Registrati', $content, ['body_class' => 'page-auth']);
