...
-- File: sql/04_auth_seed.sql
-- Gruppi, catalogo COMPLETO dei servizi (modello users-groups-services) e admin.
--
...
ON DUPLICATE KEY UPDATE
  `name` = VALUES(`name`), `description` = VALUES(`description`), `area` = VALUES(`area`),
  `path` = VALUES(`path`), `http_method` = VALUES(`http_method`),
  `is_menu_item` = VALUES(`is_menu_item`), `menu_order` = VALUES(`menu_order`), `is_active` = VALUES(`is_active`);

-- L'admin riceve tutti i servizi attivi.
INSERT IGNORE INTO `services_has_groups` (`service_id`, `group_id`)
SELECT s.`id`, g.`id` FROM `services` AS s JOIN `user_groups` AS g ON g.`code` = 'admin' WHERE s.`is_active` = 1;

-- Il proprietario riceve i servizi landlord.*
INSERT IGNORE INTO `services_has_groups` (`service_id`, `group_id`)
SELECT s.`id`, g.`id` FROM `services` AS s JOIN `user_groups` AS g ON g.`code` = 'landlord' WHERE s.`code` LIKE 'landlord.%';

-- Lo studente riceve i servizi account.*
INSERT IGNORE INTO `services_has_groups` (`service_id`, `group_id`)
SELECT s.`id`, g.`id` FROM `services` AS s JOIN `user_groups` AS g ON g.`code` = 'student' WHERE s.`code` LIKE 'account.%';

-- Amministratore iniziale.
INSERT INTO `users`
  (`email`, `password_hash`, `first_name`, `last_name`, `status`, `email_verified_at`)
VALUES
  ('admin@uniaffitti.local', '$2y$12$ZdnfvKa2yyD.Fe7ECUeFn.lzKqhoGIOwJr/yd6bsYfn2GfIwNF7fy', 'Amministratore', 'Sistema', 'active', NOW())
ON DUPLICATE KEY UPDATE `status` = VALUES(`status`);

INSERT IGNORE INTO `users_has_groups` (`user_id`, `group_id`)
SELECT u.`id`, g.`id` FROM `users` AS u JOIN `user_groups` AS g ON g.`code` = 'admin' WHERE u.`email` = 'admin@uniaffitti.local';
