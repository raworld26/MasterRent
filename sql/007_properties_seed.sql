...
-- Purpose: seed annunci allineato alla Fase 2.
...
-- Il catalogo demo e' copiato dalla Fase 2: stessi annunci, descrizioni e
-- gallerie nella cartella public/assets/uploads/case/.
...
INSERT INTO `properties`
  (`landlord_id`, `neighborhood_id`, `title`, `description`, `address`, `house_number`, `postal_code`, `total_rooms`, `has_elevator`, `heating_type`)
VALUES
...
INSERT INTO `rooms` (`property_id`, `name`, `type`, `price_monthly`, `deposit_months`, `expenses_included`, `contract_type`, `is_available`) VALUES
  (1, 'Camera singola luminosa',      'single', 250.00, 1, 0, 'Transitorio Studenti', 1),
  (2, 'Camera singola centrale',      'single', 310.00, 1, 0, 'Transitorio Studenti', 1),
  (3, 'Camera singola tranquilla',    'single', 220.00, 1, 0, 'Transitorio Studenti', 1),
  (4, 'Camera singola ben collegata', 'single', 280.00, 1, 0, 'Transitorio Studenti', 1),
  (5, 'Camera doppia condivisa',      'double', 200.00, 1, 1, 'Transitorio Studenti', 1),
  (6, 'Camera singola spaziosa',      'single', 260.00, 1, 0, 'Transitorio Studenti', 1),
  (7, 'Camera singola moderna',       'single', 290.00, 1, 0, 'Transitorio Studenti', 1);
...
) AS room_counts ON room_counts.`property_id` = p.`id`
SET p.`total_rooms` = COALESCE(room_counts.`room_count`, 0);
...
SELECT r.`id`, a.`id`
FROM `rooms` AS r
JOIN `amenities` AS a ON a.`code` IN ('wifi', 'washing_machine', 'furnished')
WHERE r.`id` IN (1, 7);

INSERT IGNORE INTO `room_has_amenities` (`room_id`, `amenity_id`)
SELECT r.`id`, a.`id`
FROM `rooms` AS r
JOIN `amenities` AS a ON a.`code` IN ('wifi', 'desk', 'furnished')
WHERE r.`id` IN (2, 3);

INSERT IGNORE INTO `room_has_amenities` (`room_id`, `amenity_id`)
SELECT r.`id`, a.`id`
FROM `rooms` AS r
JOIN `amenities` AS a ON a.`code` IN ('wifi', 'furnished')
WHERE r.`id` IN (4, 5, 6);

INSERT IGNORE INTO `room_has_amenities` (`room_id`, `amenity_id`)
SELECT r.`id`, a.`id`
FROM `rooms` AS r
JOIN `amenities` AS a ON a.`code` = 'private_bathroom'
WHERE r.`id` IN (1, 3, 6, 7);

INSERT IGNORE INTO `room_has_amenities` (`room_id`, `amenity_id`)
SELECT r.`id`, a.`id`
FROM `rooms` AS r
JOIN `amenities` AS a ON a.`code` = 'air_conditioning'
WHERE r.`id` IN (2, 7);
